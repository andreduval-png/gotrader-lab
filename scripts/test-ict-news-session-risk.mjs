...
async function main() {
  ...
  const unavailable = suite.evaluateNewsSessionRisk(signal(), {});
  assert.equal(unavailable.newsRiskLevel, "unknown");
  assert.equal(unavailable.riskGovernorAction, "downgrade_to_watchlist");
  assert.match(unavailable.newsSessionRiskNotes.join(" "), /news risk remains unknown/i);
