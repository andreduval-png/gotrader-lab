...
try {
  ...
  for (const viewport of [{ name: "desktop", width: 1440, height: 1100 }, { name: "mobile", width: 390, height: 844 }]) {
    ...
    const response = await page.goto(`${baseUrl}/dashboard?acceptanceScenario=owner-validation-policy`, { waitUntil: "domcontentloaded" });
    ...
    const coverage = page.getByTestId("operator-research-coverage");
    ...
    assert.match(coverageText, /Tech 0\/5/i);
    assert.match(coverageText, /Performance 0\/5/i);
    assert.match(coverageText, /Policy defined 5\/5/i);
    assert.equal(await rows.count(), 5);
    for (let index = 0; index < 5; index += 1) {
      const row = await rows.nth(index).innerText();
      assert.match(row, /Tech technical evidence required/i);
      assert.match(row, /Ready no/i);
    }
    assert.equal((coverageText.match(/Performance policy required/gi) ?? []).length, 0);
    assert.match(coverageText, /IFVG v3[\s\S]*Performance performance source blocked/i);
  ...
  console.log(JSON.stringify({
    status: "passed",
    scenario: "owner-validation-policy",
    technicallyValidated: 0,
    performanceValidated: 0,
    policyRequired: 0,
    researchReady: 0,
