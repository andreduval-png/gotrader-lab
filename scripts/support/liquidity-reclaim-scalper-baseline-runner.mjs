...
export const compactLegacyScanCheckpoint = (checkpoint) => {
  ...
  const unique = [...new Map(checkpoint.candidates.map((item) => [item.candidateId, item])).values()];
  return buildScanCheckpointCore({ nextSegment: checkpoint.nextSegment, candidateCount: unique.length,
    duplicateCandidateCount: checkpoint.candidates.length - unique.length, setupCount: unique.filter((item) => item.state !== "SEARCHING").length,
    expiredSetupCount: unique.filter((item) => ["SETUP_EXPIRED", "SESSION_EXPIRED"].includes(item.state)).length,
    eligibleCandidates: unique.filter((item) => item.state === "ENTRY_ELIGIBLE" && item.blockers.length === 0),
...
export async function discoverCandidates({ modules, qualified, m5, m15, parameters, checkpoint, writeCheckpoint, interruptAfterSegments,
  ...
  let candidateCount = checkpoint?.candidateCount ?? 0, duplicateCandidateCount = checkpoint?.duplicateCandidateCount ?? 0;
  let setupCount = checkpoint?.setupCount ?? 0;
  let expiredSetupCount = checkpoint?.expiredSetupCount ?? 0;
  const eligibleCandidates = [...(checkpoint?.eligibleCandidates ?? [])], seenCandidates = new Set(checkpoint?.seenCandidateIds ?? []);
  ...
  const memoryDecision = (stage = "candidate_memory_guard") => {
    const decision = classifyLrsMemoryPressure(readRssBytes(stage));
  ...
  for (; nextSegment < segmentCount; nextSegment += 1) {
    ...
    for (const event of events) {
      if (processedEventsThisProcess > 0 && memoryDecision("before_event_context") === "controlled_recycle") {
      ...
      readRssBytes("after_event_context");
      ...
      exact = undefined;
      releaseTransientMemory();
      if (memoryDecision("after_event_checkpoint_release") === "controlled_recycle") {
    ...
    events = undefined;
    releaseTransientMemory();
    readRssBytes("after_segment_release");
    ...
    if (memoryDecision("after_segment_checkpoint") === "controlled_recycle") {
      return bounded({ interrupted: true, reason: "controlled_memory_recycle", checkpoint: next });
    }
  }
...
export async function runCertifiedBaseline(input) {
  ...
  const readRssBytes = (stage = "baseline_memory_guard") => {
  ...
  readRssBytes("after_parameter_identity");
  ...
  readRssBytes("after_candidate_discovery");
  m15 = undefined;
  globalThis.gc?.();
  readRssBytes();
  ...
  globalThis.gc?.();
  if (classifyLrsMemoryPressure(readRssBytes("after_m1_index")) === "hard_limit_exceeded") throw new Error("LRS baseline exceeded the fixed 1 GiB RSS bound.");
  ...
  for (let ordinal = existing?.nextOpportunityOrdinal ?? 0; ordinal < eligible.length; ordinal += 1) {
    ...
    readRssBytes("after_bt2_record_checkpoint");
    ...
    globalThis.gc?.();
    if (classifyLrsMemoryPressure(readRssBytes("after_bt2_record_release")) === "hard_limit_exceeded") throw new Error("LRS baseline exceeded the fixed 1 GiB RSS bound.");
  }
  ...
  const reportCore = bounded({ schemaVersion: LRS_BASELINE_SCHEMA_VERSION, status: counts.blocked ? "blocked" : "passed",
    ...
    candidateCount: discovery.checkpoint.candidateCount, duplicateCandidateCount: discovery.checkpoint.duplicateCandidateCount,
    ...
    authority: LRS_BASELINE_AUTHORITY, peakRssBytes: Math.max(maximumObservedRssBytes, readRssBytes("before_final_report")) });
