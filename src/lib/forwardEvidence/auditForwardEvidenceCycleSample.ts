...
export function auditForwardEvidenceCycleSample(
  ...
  const frozenProfile = getFrozenResearchProfile(strategyProfile);
  const evaluation = evaluateForwardEvidenceLedger(
    entries,
    frozenProfile?.profileId ?? ifvgFreshRetestV3FrozenProfile.profileId
  );
  ...
  if (!frozenProfile) {
    return {
      ...
      reason: `Latest cycle used ${strategyProfile}; its simulated trades do not belong to a frozen IFVG forward ledger.`,
      nextAction: "Run a frozen IFVG research profile when needed, but count only setups issued by the live closed-candle tracker."
    };
  }
  ...
  return {
    ...
    reason: cycleTradeCount
      ...
      : `The latest ${frozenProfile.profileId} cycle produced no completed simulated trades; it contributes no forward outcomes.`,
    nextAction: `Wait for a new eligible ${frozenProfile.profileId} setup on an MT5 closed-candle event, then resolve it only with later closed candles.`
  };
}
