...
const normalizeString = (value: string, key?: string) => {
  ...
  return key && pathKeyPattern.test(key)
...
const canonicalize = (
  ...
  if (value instanceof Date) {
    throw new TypeError(
      "Canonical serialization requires explicit ISO timestamp strings, not Date objects."
    );
  }
  ...
  try {
    ...
    if (prototype !== Object.prototype && prototype !== null) {
      throw new TypeError(
        "Canonical serialization accepts only plain objects and arrays."
      );
    }
...
export async function canonicalHash(
  value: unknown,
  version = V2_CANONICAL_HASH_VERSION
) {
