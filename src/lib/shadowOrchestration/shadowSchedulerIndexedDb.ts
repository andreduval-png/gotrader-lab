...
export class IndexedDbShadowSchedulerRepository implements ShadowSchedulerRepository {
  ...
  async allQueueEntries() { const db = await open(); try { const tx = db.transaction([QUEUE], "readonly"); const values = await requestResult<ShadowScheduleQueueEntry[]>(tx.objectStore(QUEUE).getAll()); await transactionDone(tx); return values.sort((a, b) => a.queueId.localeCompare(b.queueId)); } finally { db.close(); } }
  async allReceipts() { const db = await open(); try { const tx = db.transaction([RECEIPTS], "readonly"); const values = await requestResult<ShadowDispatchReceipt[]>(tx.objectStore(RECEIPTS).getAll()); await transactionDone(tx); return values.sort((a, b) => a.dispatchedAt.localeCompare(b.dispatchedAt) || a.receiptId.localeCompare(b.receiptId)); } finally { db.close(); } }
}
