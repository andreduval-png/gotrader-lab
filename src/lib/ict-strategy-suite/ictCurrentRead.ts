...
export const buildIctCurrentReadFromPacket = (packet?: IctAdvisorPacket, latestState?: IctLatestResearchState): IctCurrentRead => {
  ...
  const reasons = uniqueReasons([
    ...
    packet.sessionRaidReversal ? `Session raid reversal: ${packet.sessionRaidReversal.status.replace(/_/g, " ")}.` : undefined,
    ...(packet.sessionRaidReversal?.missingConditions.map((reason) => `Session raid missing: ${reason}.`) ?? []),
  ...
  const currentOpportunityScan = detectCurrentOpportunities(
    buildCurrentOpportunityContext({
      ...
      currentRead: {
        ...
        sessionRaidReversal: packet.sessionRaidReversal,
